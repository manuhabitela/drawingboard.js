...
//I know.
DrawingBoard.Utils._boxBorderSize = function (
  $el,
  withPadding,
  withMargin,
  direction
...
DrawingBoard.Utils.boxBorderWidth = function ($el, withPadding, withMargin) {
  return DrawingBoard.Utils._boxBorderSize(
    $el,
    withPadding,
    withMargin,
    "width"
...
DrawingBoard.Board.prototype = {
  ...
  setMode: function (newMode, silent) {
    ...
    if (this.opts.eraserColor === "transparent")
      this.ctx.globalCompositeOperation =
        newMode === "eraser" ? "destination-out" : "source-over";
    else {
      if (newMode === "eraser") {
        if (
  ...
  fill: function (e) {
    ...
    while ((pixel = queue.pop())) {
      ...
      if (
        DrawingBoard.Utils.compareColors(pixel[COLOR], startColor, tolerance)
      ) {
        if (!useHack) updatePixelColor(pixel);
        if (pixel[X] > 0)
          // west
...
DrawingBoard.Control.Color = DrawingBoard.Control.extend({
  ...
  initialize: function () {
    ...
    $("body").on("click", function (e) {
      var $target = $(e.target);
      var $relatedButton = $target.hasClass(
      ...
      if (
        (!$relatedButton.length ||
          $relatedButton.get(0) !== $myButton.get(0)) &&
  ...
  onBoardReset: function (opts) {
    this.board.setColor(
      this.$el.find(".drawing-board-control-colors-current").attr("data-color")
    );
  },
...
DrawingBoard.Control.Navigation = DrawingBoard.Control.extend({
  ...
  initialize: function () {
    ...
    if (this.opts.back) {
      ...
      this.board.ev.bind(
        "historyNavigation",
        $.proxy(this.updateBack, this, $back)
      );
      this.$el.on(
        "click",
    ...
    if (this.opts.forward) {
      ...
      this.board.ev.bind(
        "historyNavigation",
        $.proxy(this.updateForward, this, $forward)
      );
      this.$el.on(
        "click",
